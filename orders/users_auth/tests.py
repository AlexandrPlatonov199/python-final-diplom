...
class TestViews(TestSetUP):
    ...
    def test_user_details(self):
        ...
        resp_get = self.client.get(self.details_url, headers={'Authorization': f'Token {self.auth_token}'},
                                   format='json')
        resp_retrieve = self.client.get(self.details_url + str(user.id) + '/',
                                        headers={'Authorization': f'Token {self.auth_token}'}, format='json')
        resp_post = self.client.post(self.details_url + str(user.id) + '/' + 'change/',
                                     data={'password': '12wvfus89'},
                                     headers={'Authorization': f'Token {self.auth_token}'}, format='json')
